...
# java-profile.sh - Automated Java profiling script with async-profiler v4.0
...
# Step 2: Set profiler directory (relative to script location)
...
download_profiler() {
    ...
    if [ ! -d "$profiler_dir/current" ]; then
        ...
        rm "$filename"  # Clean up downloaded archive
        cd - > /dev/null
...
show_profiling_menu() {
    ...
    echo "9. Memory Leak Detection (5min)"
...
execute_profiling() {
    ...
    case $option in
        ...
        5)
            ...
            if [ -f "$PROFILER_DIR/current/bin/jfrconv" ]; then
                ...
                echo -e "${YELLOW}jfrconv not available, using jfr2flame instead${NC}"
                "$PROFILER_DIR/current/bin/jfr2flame" "$JFR_FILE" "$HEATMAP_FILE"
            fi
            ;;
        6)
        ...
        8)
            ...
            if [[ "$DURATION" =~ ^[0-9]+$ ]] && [ "$DURATION" -gt 0 ]; then
                ...
                echo -e "${RED}Invalid duration. Please enter a positive number.${NC}"
                return
            fi
            ;;
        9)
            echo -e "${GREEN}Starting memory leak detection for 5 minutes...${NC}"
            echo -e "${YELLOW}This will profile heap allocations to detect memory leaks${NC}"
    ...
    if [ $option -ne 10 ] && [ $option -ne 0 ]; then
        ...
        # Show how to open the results
        echo -e "${BLUE}To view results in browser:${NC}"
        echo "  open $RESULTS_DIR/$(ls -t "$RESULTS_DIR"/*.html 2>/dev/null | head -1 | xargs basename 2>/dev/null || echo 'latest-file.html')"
    fi
}
...
while true; do
    ...
    show_profiling_menu
    read -p "Select profiling option (0-10): " PROFILE_TYPE
