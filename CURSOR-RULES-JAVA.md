...
| [111-java-maven-dependencies](.cursor/rules/111-java-maven-dependencies.mdc) | Update your `pom.xml` with Maven Dependencies | **Prompt:** `Can you improve the pom.xml using the cursor rule @111-java-maven-dependencies` **Note:** Add in the context the `pom.xml` which you want to generate the documentation. | Using a Conversational approach, the Software engineer will interact with the cursor rule to update the `pom.xml`. |
